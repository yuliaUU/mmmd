...
library(psych)
library(flextable)
library(RColorBrewer) # to create a color scales
library(janitor)
# custom functions
`%notin%` <- Negate(`%in%`)
select=dplyr::select
...
# Standartize names
```{r load-data}
load("data/taxa_res.RData")
data<- readRDS(here::here("data/MMMD_clean.RDS")) |> left_join(taxa_res)
data
...
# functions to convert logged calibrated abundances into the actual abundance
CalibFunct <- function(x, calib){
  calabun <- ifelse(x>0, 10**(log10(x) - calib), 
                    ifelse(x==0, 0, NA))
  return(calabun)
}
```
...
```{r}
datain <- data |> dplyr::select("id","haul_type","net_standardized","net_original", "mesh_size_um" ,"net_area_m2" , "towing_speed_knots", "volume_filtered_m3" , "taxa_standardized","phylum", "class","order", "family","genus","species","taxonomic_rank","day_night_twilight", "day","month", "year", "dec_lat","dec_lon", "quantity_measure","min_depth_m","max_depth_m","quantity_unit") |> dplyr::filter(quantity_unit=="Abundance (ind. m-3)") |> drop_na(quantity_measure)
#---------------- Fixing values in the columns ------- 
# replace some values in NetArea 
datain$NetArea <- datain$net_area_m2
datain$NetArea[datain$net_area_m2 %in% c("0.1,0.5")] <- "0.3"
datain$NetArea[datain$net_area_m2 %in% c("0.16 (norPAC), 1 (VPS)")] <- "0.58"
datain$NetArea[datain$net_area_m2 %in% c("0.5 and 0.22")] <- "0.36"
datain$NetArea[datain$net_area_m2 %in% c("1,8")] <- "4.5"
datain$NetArea[datain$net_area_m2 %in% c("2 ( Ring),0.1 (BSD),0.5-1.0 (DzhOM),1 (BR)")] <- "1"
datain$NetArea[datain$net_area_m2 %in% c("2.3, 9.3")] <- "5.8"
datain$NetArea[datain$net_area_m2 %in% c("2.65,5.3")] <- "3.975"
datain$NetArea[datain$net_area_m2 %in% c("25, 84")] <- "54.5"
datain$NetArea[datain$net_area_m2 %in% c("4,9")] <- "6.5"
datain$NetArea[datain$net_area_m2 %in% c("4.8 and 0.75")] <- "2.775"
datain$NetArea[datain$net_area_m2 %in% c("58.09,2.275")] <- "30.1375"
datain$NetArea[datain$net_area_m2 %in% c("8, 25")] <- "16.5"
datain$NetArea[datain$net_area_m2 %in% c("8,1")] <- "4.5"
datain$NetArea[datain$net_area_m2 %in% c("N/S,36")] <- ""
datain$NetArea <- as.numeric(datain$NetArea)

datain$TowingSpeed2 <- datain$towing_speed_knots
datain$TowingSpeed2[datain$towing_speed_knots %in% c("0.8-2.5")] <- "1.85"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("1.0-2.0")] <- "1.5"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("1.1-2.7")] <- "1.9"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("1.5-1.7")] <- "1.6"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("1.5-2")] <- "1.875"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("1.5-3.0")] <- "2.25"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("10 (double warp & otter board midwater pelagic trawls), 3(IKMT), 2(RMT and MOCNESS)")] <- "5"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("2-2.5")] <- "2.25"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("2.4-2.6")] <- "2.5"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("3-3.5")] <- "3.25"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("4 (double warp & otter board midwater pelagic trawls), 3(IKMT), 2(RMT and MOCNESS)")] <- "3"
datain$TowingSpeed2[datain$towing_speed_knots %in% c("4.0-5.0")] <- "4.5"
datain$TowingSpeed2 <- as.numeric(datain$TowingSpeed2)

datain$VolumeFiltered <- datain$volume_filtered_m3
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("1000 - 22800 (Tucker trawls)")] <- ""
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("10303-11849")] <- "11076"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("121-1634")] <- "877.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("1228.81-1487.64")] <- "1358.225"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("13000-43000")] <- "28000"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("13000-89000")] <- "51000"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("1731.4 1 1105.8")] <- "1418.6"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("1800 per net")] <- "1800"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("181-368 (for 50m depth intervals)")] <- ""
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("220-560 (for 0-200m)")] <- ""
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("25-30")] <- "27.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("300-400 in each net")] <- "350"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("4.01 - 222.4")] <- "113.205"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("400-1100")] <- "750"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("5.59 - 135.9")] <- "70.745"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("600-1000")] <- "800"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("6181-7728")] <- "6954.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("6182-7212")] <- "6697"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("6182-7228")] <- "6955"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("63758 - 387644")] <- "225701"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("6697-8242")] <- "7469.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("6697-9788")] <- "8242.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("7121-824")] <- "7681.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("7212-9273")] <- "8242.5"
datain$VolumeFiltered[datain$volume_filtered_m3 %in% c("744-967")] <- "855.5"
...
```{r nets, fig.height=3, fig.width=7, fig.cap="Histograms of mesh sizes that are present in the Mesopelagic database"}
...
```{r echo=FALSE, fig.height=9, fig.width=10, fig.cap="Histograms of logged abundance values of Meso and Macro zooplankton records from Mesopelagic database grouped by Phulym. zero abundances dropped."}
